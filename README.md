...
parameters:
  user_id: [1, 2]       # 优先级 4（会被参数化覆盖）

steps:
  - name: 登录
    extract:
      user_id: $.data.id  # 优先级 5（从响应提取）

  - name: 获取用户
    variables:
      user_id: 999      # 优先级 2（步骤级最高）
    request:
...
**特性**：
- 📈 **摘要仪表板**：总数、通过、失败、跳过、耗时
- 🔍 **详细断言**：每个断言的期望值、实际值、结果
- 📦 **请求/响应**：完整的请求和响应数据
- 🎛️ **交互过滤**：隐藏已通过用例、搜索功能
- 🎨 **暗色主题**：开发者友好的视觉设计
...
**原因**：`arun_hooks.py` 文件位置不正确。

**解决方案**：
- 将 `arun_hooks.py` 放在项目根目录，或
- 使用环境变量指定路径：
...
**自定义函数**：在 `arun_hooks.py` 中定义任意 Python 函数即可在模板中使用。
