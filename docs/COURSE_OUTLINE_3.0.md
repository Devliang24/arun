...
15. 规范化校验：用 `arun check` 校验用例通过、`arun fix` 一键修复常见风格问题；提交前通过本地校验脚本。
16. 阅读和修改源码：完成一次小型修复或扩展（<50 行），`arun check` 通过且示例回归全绿（提交修改文件与验证命令）。
...
11. 规范化工具：`arun check` 校验（语法/提取前缀/断言目标/Hook 命名/空行），`arun fix` 自动修复（`--only-spacing/--only-hooks`）。
...
14. 资产转换引导：先导 `arun convert` 基本用法，明确导入后需手动补充断言/提取与变量占位。
...
8. Hooks 机制：`setup_hooks/teardown_hooks` 调用顺序；`hook_ctx` 可用信息；HMAC/时间戳签名示例（`arun_hooks.py`、`arun/loader/hooks.py`）。
...
11. 性能阈值与基线：`$elapsed_ms` 阈值设计、P95 监控与回归比对（`arun/utils/timeit.py`）。
12. SQL 校验与结果复用：查询断言、金额/库存等典型场景；变量存储与后续步骤复用（`arun/db/sql_validate.py`、`examples/test_sql_*`）。
...
4. 结构化日志与脱敏：`arun/utils/logging.py`、`arun/utils/mask.py`（`--mask-secrets` 与 `reveal_secrets`）。
...
## 阶段 6｜源码拆解与扩展
1. CLI 与命令体系：`arun/cli.py`（run/convert/export/check/fix、过滤/报告/通知参数）。
